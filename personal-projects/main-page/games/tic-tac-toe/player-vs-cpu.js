...
function startGame() {
    ...
    gameLogsElement.innerHTML = ''; // Clear logs
}
...
function playerMove(cellIndex) {
    ...
    if (checkWinner(currentPlayer)) {
        ...
        logGame(`${currentPlayer} wins the game!`);
        ...
        if (currentPlayer === 'O') {
            setTimeout(cpuMove, 500); // CPU move with a delay
        }
    }
}
...
function cpuMove() {
    ...
    updateBoard();
    logGame(`CPU placed on ${randomIndex + 1}`);

    if (checkWinner('O')) {
        gameOver = true;
        statusDisplay.textContent = 'CPU wins!';
        logGame("CPU wins the game!");
    } else if (gameBoard.every(cell => cell !== '')) {
        gameOver = true;
        statusDisplay.textContent = "It's a Draw!";
        logGame("It's a draw!");
